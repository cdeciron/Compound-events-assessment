...
for region in regions:
    ...
    for period in period_names:
        ...
        plt.hist(counts, bins=15, alpha=0.5, color=period_colors[period],
                 label=period_names[period])#, edgecolor='black')

        # Threshold line
        plt.axvline(thresholds[period][region], color=period_colors[period], linestyle='--', linewidth=2)

    plt.title(f'Bootstrap Distribution of Compound Events - Region {region}')
    plt.xlabel('Number of Compound Events')
    ...
    plt.tight_layout()
    plt.savefig(f'{output_path}/{region}_CE_Bootstrap.png', dpi=300)
    plt.show()
