...
for period_key, period_code in period_names.items():
    ...
    # If a day is 'dark': labeled 'D'. If it's not, labeled 'L'. Solar data goes from 3hourly to daily
    solar_labels = {}
    ...
    # Apply the power law to the current wind_df. Wind data goes from 10m height to 100m height
    df_wind_100m = power_law(wind_df)  
    df_wind_100m = df_wind_100m.rename(columns={zone: f"{zone}_wind" for zone in df_wind_100m.columns})
    
    # Merge wind daily data and solar daily data in the same df 
...
def compound_events(df, wind_thresh=4, min_spell_length=5):
    ...
    for zone in zones:
        ...
        # Find the compound events based on the dates where each hazard occurs
        #One compound event = > at least overlap of dark spell and low wind spell 
